...
export class FFArchive {
    ...
    constructor(blob, index) {
        ...
        // if (imageFile) {
        //     this.animationFrames = readAnimationFrameRegistry(imageFile.content);
        //     console.log('[-IMAGES.OPT] Animation frames registry', this.animationFrames);
        //     //this.animations = readAnimationFrameIndex(indexFile.content, animFile.content, imageFile.content);
        // }
    }
}
...
function readAnimationSequenceRegistry(animSeqContent) {
    const stream = new BinaryStream(animContent, 0 , ByteOrder.littleEndian);
    const sequences = {};
}
...
//  * @param indexContent {Uint8Array}
//  * @param animContent {Uint8Array}
//  * @param imagesContent {Uint8Array}
//  */
// function readAnimationFrameIndex(indexContent, animContent, imagesContent) {
//     let stream = new BinaryStream(indexContent, 0, ByteOrder.littleEndian);
//     const animationCount = stream.read4();
//
//     /** @type {ImageAnimation[]} */const imageAnimations = new Array(animationCount);
//     for (let i = 0; i < animationCount; i++) {
//         const id = stream.read4();
//         const imageName = stream.readNullTermString();
//         stream.index += 8;
//
//         imageAnimations[i] = new ImageAnimation(id, imageName);
//     }
//
//     stream = new BinaryStream(animContent, 0 , ByteOrder.littleEndian);
//     const animations = [];
//     let animationNumber = 0;
//     while (!stream.isComplete) {
//         const sequenceNumber = stream.read4();
...
//     }
//
//     for (let imageAnimation of imageAnimations) {
//         const animationNumber = animations.find(x => x.name === imageAnimation.name);
//         if (animationNumber) {
//             imageAnimation.animationNumber = animationNumber.animationNumber;
//             imageAnimation.sequenceNumber = animationNumber.sequenceNumber;
//         }
//     }
//
//     stream = new BinaryStream(imagesContent, 0, ByteOrder.littleEndian);
//
// }
...
export class DbfTable {
    ...
    /** 
     * @public
     * @param data {Record<string, *>|Array}
     * */
    addRecord(data) {
        const record = new DbfTableRecord(null, this);
        if (Array.isArray(data)) {
            data.forEach((value, index) => record.set(index, value));
        } else {
            record.set(data);
        }
...
class DbfTableHeader {
    ...
    /**
     * Returns DBF table version info
     * @public
     * @returns {string}
     */
    getVersionInfo() {
...
function readFieldValue(stream, column) {
    switch (column.fieldType) {
        case FieldTypes.character:
            return stream.readString(column.fieldSize).trimEnd();

        case FieldTypes.date:
            const dateStr = stream.readString(8);
            const datePart = dateStr.substring(0, 4);
            const monthPart = dateStr.substring(4, 2);
            const dayPart = dateStr.substring(6, 2);
...
function padLeft(stream, value, length, padChar) {
    const firstCharIndex = value.length > length
        ? length - value.length
        : 0;
    const result = firstCharIndex === 0 ? value : value.substring(firstCharIndex);
